...
void AutomationEngine::startRunning(View* view, Settings* settings) {
    mIsRunning = true;
    mCurrentTest = 0;
    mElapsedTime = 0;
    mElapsedFrames = 0;
...
void AutomationEngine::requestBatchMode() {
    mBatchModePending = true;
...
void AutomationEngine::tick(View* view, Renderer* renderer, float deltaTime, Settings* settings) {
    if (!mIsRunning) {
        if (mBatchModeAllowed && mBatchModePending) {
            startRunning(view, settings);
            mBatchModePending = false;
        }
        return;
    }
    ...
    if (mOptions.exportSettings) {
        ...
        exportSettings(*settings, filename.c_str());
    }
